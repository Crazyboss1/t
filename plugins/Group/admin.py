...
@Tiger.on_message(command("adminlist"))
...
@Tiger.on_message(command("admincache"))
...
@Tiger.on_message(filters.regex(r"^(?i)@admin(s)?") & filters.group)
...
@Tiger.on_message(command("fullpromote") & promote_filter)
async def fullpromote_usr(c: Tiger, m: Message):
    global ADMIN_CACHE
...
@Tiger.on_message(command("promote") & promote_filter)
async def promote_usr(c: Tiger, m: Message):
...
@Tiger.on_message(command("demote") & promote_filter)
async def demote_usr(c: Tiger, m: Message):
...
@Tiger.on_message(command("invitelink"))
async def get_invitelink(c: Tiger, m: Message):
...
@Tiger.on_message(command("setgtitle") & admin_filter)
...
@Tiger.on_message(command("setgdes") & admin_filter)
...
@Tiger.on_message(command("title") & admin_filter)
async def set_user_title(c: Tiger, m: Message):
...
@Tiger.on_message(command("setgpic") & admin_filter)
async def setgpic(c: Tiger, m: Message):
    user = await m.chat.get_member(m.from_user.id)
    if not user.can_change_info and user.status != "creator":
        await m.reply_text(
            "You don't have enough permission to use this command!",
